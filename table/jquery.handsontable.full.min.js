...
})(jQuery, window, Handsontable); (function () { var j, o = "1.4.7", h = {}, a = "en", p = null, g = (typeof module !== "undefined" && module.exports); function k(q) { this._n = q } function l(v, u, q) { var t = Math.pow(10, u), s; s = (Math.round(v * t) / t).toFixed(u); if (q) { var r = new RegExp("0{1," + q + "}$"); s = s.replace(r, "") } return s } function d(r, q) { var s; if (q.indexOf("$") > -1) { s = b(r, q) } else { if (q.indexOf("%") > -1) { s = e(r, q) } else { if (q.indexOf(":") > -1) { s = f(r, q) } else { s = c(r, q) } } } return s } function m(t, w) { if (w.indexOf(":") > -1) { t._n = n(w) } else { if (w === p) { t._n = 0 } else { var x = w; if (h[a].delimiters.decimal !== ".") { w = w.replace(/\./g, "").replace(h[a].delimiters.decimal, ".") } var y = new RegExp(h[a].abbreviations.thousand + "(?:\\)|(\\" + h[a].currency.symbol + ")?(?:\\))?)?$"), s = new RegExp(h[a].abbreviations.million + "(?:\\)|(\\" + h[a].currency.symbol + ")?(?:\\))?)?$"), q = new RegExp(h[a].abbreviations.billion + "(?:\\)|(\\" + h[a].currency.symbol + ")?(?:\\))?)?$"), z = new RegExp(h[a].abbreviations.trillion + "(?:\\)|(\\" + h[a].currency.symbol + ")?(?:\\))?)?$"); var v = ["KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"], r = false; for (var u = 0; u <= v.length; u++) { r = (w.indexOf(v[u]) > -1) ? Math.pow(1024, u + 1) : false; if (r) { break } } t._n = ((r) ? r : 1) * ((x.match(y)) ? Math.pow(10, 3) : 1) * ((x.match(s)) ? Math.pow(10, 6) : 1) * ((x.match(q)) ? Math.pow(10, 9) : 1) * ((x.match(z)) ? Math.pow(10, 12) : 1) * ((w.indexOf("%") > -1) ? 0.01 : 1) * Number(((w.indexOf("(") > -1) ? "-" : "") + w.replace(/[^0-9\.-]+/g, "")); t._n = (r) ? Math.ceil(t._n) : t._n } } return t._n } function b(r, q) { var t = (q.indexOf("$") <= 1) ? true : false; var u = ""; if (q.indexOf(" $") > -1) { u = " "; q = q.replace(" $", "") } else { if (q.indexOf("$ ") > -1) { u = " "; q = q.replace("$ ", "") } else { q = q.replace("$", "") } } var s = d(r, q); if (t) { if (s.indexOf("(") > -1 || s.indexOf("-") > -1) { s = s.split(""); s.splice(1, 0, h[a].currency.symbol + u); s = s.join("") } else { s = h[a].currency.symbol + u + s } } else { if (s.indexOf(")") > -1) { s = s.split(""); s.splice(-1, 0, u + h[a].currency.symbol); s = s.join("") } else { s = s + u + h[a].currency.symbol } } return s } function e(r, q) { var t = ""; if (q.indexOf(" %") > -1) { t = " "; q = q.replace(" %", "") } else { q = q.replace("%", "") } r._n = r._n * 100; var s = d(r, q); if (s.indexOf(")") > -1) { s = s.split(""); s.splice(-1, 0, t + "%"); s = s.join("") } else { s = s + t + "%" } return s } function f(t, q) { var r = Math.floor(t._n / 60 / 60), s = Math.floor((t._n - (r * 60 * 60)) / 60), u = Math.round(t._n - (r * 60 * 60) - (s * 60)); return r + ":" + ((s < 10) ? "0" + s : s) + ":" + ((u < 10) ? "0" + u : u) } function n(r) { var s = r.split(":"), q = 0; if (s.length === 3) { q = q + (Number(s[0]) * 60 * 60); q = q + (Number(s[1]) * 60); q = q + Number(s[2]) } else { if (s.lenght === 2) { q = q + (Number(s[0]) * 60); q = q + Number(s[1]) } } return Number(q) } function c(y, u) { var A = false, B = false, q = "", s = "", C = "", r = Math.abs(y._n); if (y._n === 0 && p !== null) { return p } else { if (u.indexOf("(") > -1) { A = true; u = u.slice(1, -1) } if (u.indexOf("a") > -1) { if (u.indexOf(" a") > -1) { q = " "; u = u.replace(" a", "") } else { u = u.replace("a", "") } if (r >= Math.pow(10, 12)) { q = q + h[a].abbreviations.trillion; y._n = y._n / Math.pow(10, 12) } else { if (r < Math.pow(10, 12) && r >= Math.pow(10, 9)) { q = q + h[a].abbreviations.billion; y._n = y._n / Math.pow(10, 9) } else { if (r < Math.pow(10, 9) && r >= Math.pow(10, 6)) { q = q + h[a].abbreviations.million; y._n = y._n / Math.pow(10, 6) } else { if (r < Math.pow(10, 6) && r >= Math.pow(10, 3)) { q = q + h[a].abbreviations.thousand; y._n = y._n / Math.pow(10, 3) } } } } } if (u.indexOf("b") > -1) { if (u.indexOf(" b") > -1) { s = " "; u = u.replace(" b", "") } else { u = u.replace("b", "") } var F = ["B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"], x, v; for (var D = 0; D <= F.length; D++) { x = Math.pow(1024, D); v = Math.pow(1024, D + 1); if (y._n >= x && y._n < v) { s = s + F[D]; if (x > 0) { y._n = y._n / x } break } } } if (u.indexOf("o") > -1) { if (u.indexOf(" o") > -1) { C = " "; u = u.replace(" o", "") } else { u = u.replace("o", "") } C = C + h[a].ordinal(y._n) } if (u.indexOf("[.]") > -1) { B = true; u = u.replace("[.]", ".") } var H = y._n.toString().split(".")[0], E = u.split(".")[1], G = u.indexOf(","), t = "", z = false; if (E) { if (E.indexOf("[") > -1) { E = E.replace("]", ""); E = E.split("["); t = l(y._n, (E[0].length + E[1].length), E[1].length) } else { t = l(y._n, E.length) } H = t.split(".")[0]; if (t.split(".")[1].length) { t = h[a].delimiters.decimal + t.split(".")[1] } else { t = "" } if (B && Number(t) === 0) { t = "" } } else { H = l(y._n, null) } if (H.indexOf("-") > -1) { H = H.slice(1); z = true } if (G > -1) { H = H.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1" + h[a].delimiters.thousands) } if (u.indexOf(".") === 0) { H = "" } return ((A && z) ? "(" : "") + ((!A && z) ? "-" : "") + H + t + ((C) ? C : "") + ((q) ? q : "") + ((s) ? s : "") + ((A && z) ? ")" : "") } } j = function (q) { if (j.isNumeral(q)) { q = q.value() } else { if (!Number(q)) { q = 0 } } return new k(Number(q)) }; j.version = o; j.isNumeral = function (q) { return q instanceof k }; j.language = function (q, r) { if (!q) { return a } if (q && !r) { a = q } if (r || !h[q]) { i(q, r) } return j }; j.language("en", { delimiters: { thousands: ",", decimal: "." }, abbreviations: { thousand: "k", million: "m", billion: "b", trillion: "t" }, ordinal: function (r) { var q = r % 10; return (~~(r % 100 / 10) === 1) ? "th" : (q === 1) ? "st" : (q === 2) ? "nd" : (q === 3) ? "rd" : "th" }, currency: { symbol: "$" } }); j.zeroFormat = function (q) { if (typeof (q) === "string") { p = q } else { p = null } }; function i(q, r) { h[q] = r } j.fn = k.prototype = { clone: function () { return j(this) }, format: function (q) { return d(this, q ? q : j.defaultFormat) }, unformat: function (q) { return m(this, q ? q : j.defaultFormat) }, value: function () { return this._n }, valueOf: function () { return this._n }, set: function (q) { this._n = Number(q); return this }, add: function (q) { this._n = this._n + Number(q); return this }, subtract: function (q) { this._n = this._n - Number(q); return this }, multiply: function (q) { this._n = this._n * Number(q); return this }, divide: function (q) { this._n = this._n / Number(q); return this }, difference: function (r) { var q = this._n - Number(r); if (q < 0) { q = -q } return q } }; if (g) { module.exports = j } if (typeof ender === "undefined") { this["numeral"] = j } if (typeof define === "function" && define.amd) { define([], function () { return j }) } }).call(this);